<script>
    ...
    $: listPromise = (searchKeyword)(keyword, listCurrentPage);
    ...
    async function searchKeyword(keyword = "", currentPage = 1) {
        ...
        const res = await fetchServer(`vendors?keyword=${keyword}&page=${currentPage}`, {
                    ...
                    {#if payload.length > 0}
                        <div class="list-group mt-3 mb-2">
                            {#each payload as item}
                                <a on:click={() => handleItemClick(item)} href="#top" class="list-group-item list-group-item-action">{item.name}</a>
                            {/each}
                        </div>
                        <Pagination
